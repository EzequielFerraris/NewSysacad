...
namespace NewSysacadFront
{
    public partial class FrmAgregarProfesor : Form
    {
        ...
        public FrmAgregarProfesor(Administrador admin, FrmListaProfesores listaPadre)
        {
            InitializeComponent();
            this.admin = admin;
            ...
            if (result == DialogResult.Yes)
            {
                ...
                foreach (string prop in props)
                {
                    if (prop == string.Empty)
                    {
                        ...
                        string tituloError1 = "Agregar Profesor";
                ...
                if (!nuevoProfe.ChequearMailsDuplicados(nuevoProfe.EMail))
                {
                    validados = false;

                    string mensajeError1 = $"El email ingresado ya se encuentra en uso.";
                    string tituloError1 = "Agregar Profesor";
                    DialogResult confirmacion = MessageBox.Show(mensajeError1, tituloError1);
                ...
                if (validados)
                {
                    try
                    {
                        if (nuevoProfe.AgregarABD(out string error))
                        {
                            agregado = true;
                            this.listaPadre.ActualizarLista();
                            string mensaje1 = "Profesor agregado correctamente.";
                            string titulo1 = "Agregar Profesor";
                    ...
                    catch
                    {
                        ...
                        string titulo2 = "Agregar Profesor";
